...
const ShowModal = ({
  ...
  const getData = async () => {
    try {
      ...
      const { data } = await axios.get(
        `${config.endpoint}/movies/get-all-movies`
      );
      if (data.success) {
        setMovies(data.data);
      } else {
      ...
      const { data: showData } = await axios.post(
        `${config.endpoint}/shows/get-all-shows-by-theatre`,
        {
      ...
      if (showData.success) {
        ...
        message.error(showData.message);
      }
  ...
  const onFinish = async (values) => {
    try {
      ...
      let response = null;
      if (view === "form") {
        response = await axios.post(`${config.endpoint}/shows/add-show`, {
          ...values,
          theatre: selectedTheatre._id,
        });
      } else {
        response = await axios.put(`${config.endpoint}/shows/update-show`, {
          ...values,
  ...
  const handleDelete = async (showId) => {
    try {
      ...
      const { data } = await axios.post(
        `${config.endpoint}/shows/delete-show`,
        {
          showId,
        }
      );
